...
import asyncio
import logging
from api.config import EMAIL_COLUMN, VERIFIED_SHEET_NAME  # Import EMAIL_COLUMN from the config module

# Configure logging
logger = logging.getLogger(__name__)

async def verify_emails(sheet_name: str, get_sheet_data, update_sheet_data, delete_sheet_rows):
        ...
        for i in range(0, len(rows), batch_size):
            ...
            for index, response in enumerate(responses):
                try:
                    ...
                    rows_to_delete.append(i + index + 1)  # +1 because row indices are 0-based, and we removed the header row
